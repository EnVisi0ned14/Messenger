...
//  Messanger
//
//  Created by Michael Abrams on 10/31/21.
//
...
extension DatabaseManager {
    ...
    public func createNewConversation(with otherUserEmail: String, firstMessage: Message, name: String, completion: @escaping (Bool) -> Void) {
        guard let currentEmail = UserDefaults.standard.value(forKey: "email") as? String,
            ...
            print("failed to grab user, name and email")
            return
        }
        ...
        ref.observeSingleEvent(of: .value, with: { [weak self] snapshot in
            ...
            if var conversations = userNode["conversations"] as? [[String: Any]] {
                //conversation array exits for current user
    ...
    public func getAllMessagesForConversation(with id: String, completion: @escaping (Result<[Message], Error>) -> Void) {
        database.child("\(id)").observe(.value, with: { snapshot in
            guard let messageNode = snapshot.value as? [String: Any] else {
                completion(.failure(DatabaseError.failedToFetch))
                return
            }
            
            if let safeMessagesNode = messageNode["messages"] as? [[String: Any]] {
                
                let messages: [Message] = safeMessagesNode.compactMap({ dictionary in
                ...
                print("Failed to create user node")
            }
    ...
    ///Sends a mesage with target conversation and message
